...
Page({
    ...
    jia: function(t) {
        ...
        if (v.num > k[0].num) {
            ...
            var w = a.data.quick_hot_goods_lists, x = w.find(function(t) {
                return t.id == o.id;
            });
            x && (x.num += 1), a.setData({
                quick_list: r,
    ...
    jian: function(t) {
        ...
        v.num -= 1, v.goods_price = parseFloat(v.goods_price), v.goods_price -= g[0].price, 
    ...
    guigejian: function(t) {
        ...
        var m = a.data.quick_hot_goods_lists, v = m.find(function(t) {
            return t.id == o.id;
        });
        v && (v.num -= 1), a.setData({
        ...
        var k = a.data.attr_group_list, S = [];
        for (var n in k) {
            var q = !1;
            for (var w in k[n].attr_list) if (k[n].attr_list[w].checked) {
                q = {
                    attr_id: k[n].attr_list[w].attr_id,
                    attr_name: k[n].attr_list[w].attr_name
                };
                break;
            }
            if (!q) return wx.showToast({
                title: "请选择" + k[n].attr_group_name,
                image: "/images/icon-warning.png"
            }), !0;
            S.push({
                attr_group_id: k[n].attr_group_id,
                attr_group_name: k[n].attr_group_name,
                attr_id: q.attr_id,
                attr_name: q.attr_name
            });
        }
        for (var x = a.data.check_num, D = a.data.carGoods, s = D.length, g = 0; g < s; g++) D[g].goods_id == i && JSON.stringify(D[g].attr) == JSON.stringify(S) && (D[g].num -= 1, 
        x = D[g].num);
        a.setData({
            check_num: x
        });
        var y = a.data.total;
        y.total_num -= 1;
        var F = a.data.check_goods_price;
        F = parseFloat(F), y.total_price = parseFloat(y.total_price), y.total_price -= F, 
        y.total_price = y.total_price.toFixed(2), a.setData({
            total: y
        }), 0 == y.total_num && a.setData({
            goodsModel: !1
        });
    },
    ...
    tianjia: function(t) {
        ...
        var F = a.data.quick_hot_goods_lists, G = F.find(function(t) {
            return t.id == o.id;
        });
        G && (G.num += 1), a.setData({
            quick_list: r,
